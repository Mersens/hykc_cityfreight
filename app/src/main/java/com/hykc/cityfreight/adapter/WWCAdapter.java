...
public class WWCAdapter extends RecyclerView.Adapter<WWCAdapter.ViewHolder> {
    ...
    public WWCAdapter(Context mContext, List<UWaybill> list){
    ...
    @NonNull
    @Override
    public WWCAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view=mInflater.inflate(R.layout.layout_dfh_item,viewGroup,false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(WWCAdapter.ViewHolder holder, final int i) {
    ...
    private void setBtnData(ViewHolder holder, final UWaybill uWaybill, final int pos){
        ...
        switch (status){
            ...
            case WaybillStatus.YJD_STATUS://已接单
                //配送
                holder.mTextCancel.setVisibility(View.GONE);
                holder.mImgStutuType.setImageResource(R.mipmap.icon_statu_ps_gren);
                holder.mImgStutuType.setVisibility(View.VISIBLE);
                holder.mImgStutuType.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
